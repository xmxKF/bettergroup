...
| `common.json` `shell.gateway_note` | "Selecting your language…" → "Choose your language". The gateway's `meta refresh` fallback was removed (WCAG 2.2.1 / F40), so with JS off the page no longer redirects — a progress phrase would have described something that never happens. Imperative works for both cases. |
